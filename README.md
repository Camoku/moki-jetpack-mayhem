...
- **Powerup art (Magnet, Doubler, Shield)** — `sprites/powerups/`. `Powerup.tscn` is still "one scene,
  ...
  `x2_frames.tres` (a single static `idle` icon); **Shield** = `shield_frames.tres` (static
  `idle`). Adding more powerup art = one row in `SPRITE_ART`. (The source files were JPEGs with a
